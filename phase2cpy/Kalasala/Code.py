...
def Bill(PID):
	...
	if TID[0][0] == None:
		...
		TID = int(TID[0][0])
	execute_query("select sum(Price) from Medicine where Medicine_ID in (select Medicine_ID from Bill where Treatment_ID = " + str(TID) + ");",1)
	print("Room Bill :")
	# print("select Cost from Rooms where Room_ID=(select Room_ID from Patient_1 where Patient_ID = " + str(PID) + ");")
	execute_query("select Cost from Rooms where Room_ID=(select Room_ID from Patient_1 where Patient_ID = " + str(PID) + ");",1)
